...
class Input {
 public:
  ...
  /// 判断是否按了鼠标某个按钮
  /// \param mouse_button_index   0 表示主按钮（通常为左按钮），1 表示副按钮，2
  /// 表示中间按钮。 \return
  ...
  /// \param x
  /// \param y
  static void set_mousePosition(float x, float y) {
    mouse_position_.x = x;
    mouse_position_.y = y;
  }
 ...
 private:
  static std::unordered_map<unsigned short, unsigned short>
      key_event_map_;  // 按键状态 奇数按下 偶数弹起 0表示没有记录
