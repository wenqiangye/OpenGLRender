...
bool CreateDeviceD3D(HWND hWnd) {
  ...
  if (g_pd3dDevice->CreateCommandList(
          ...
          NULL, IID_PPV_ARGS(&g_pd3dCommandList)) != S_OK ||
...

  {
    ...
    if (dxgiFactory->CreateSwapChainForHwnd(g_pd3dCommandQueue, hWnd, &sd, NULL,
                                            NULL, &swapChain1) != S_OK)
      return false;
...
class App {
 public:
  App() {
    ...
    ::RegisterClassEx(&wc);
    hwnd = ::CreateWindow(wc.lpszClassName, _T("Dear ImGui DirectX12 Example"),
